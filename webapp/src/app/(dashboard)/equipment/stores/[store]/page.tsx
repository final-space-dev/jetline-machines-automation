  ...
  return (
    <AppShell>
      ...
      <div className="jl-page-pad" style={{ padding: "28px 40px 60px" }}>
        ...
        {/* Data completeness card (P19) */}
        {completeness && (
          <div className="jl-card jl-card--pad-sm" style={{ marginBottom: 24, display: "flex", alignItems: "center", gap: 28, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              <span className="jl-display" style={{ color: completenessColors(completeness.overallScore).color }}>
                {Math.round(completeness.overallScore)}%
              </span>
              <span className="jl-sm jl-muted" style={{ fontWeight: 600 }}>Data Complete</span>
            </div>
            <span className="jl-badge">Equipment {Math.round(completeness.equipmentScore)}%</span>
            <span className="jl-badge">Printers {Math.round(completeness.printerScore)}%</span>
            <Link
              href={`/stores/${encodeURIComponent(storeName)}/completeness`}
                    ...
                    filtered.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ color: item.located_at ? "var(--ink-600)" : "var(--ink-400)", fontStyle: item.located_at ? "normal" : "italic" }}>
                            {item.located_at ?? "Not set"}
                          </td>
                        </tr>
                      );
                    })
